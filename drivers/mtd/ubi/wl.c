...
#ifdef CONFIG_MTK_SLC_BUFFER_SUPPORT
			ubi_err("No free blocks, works_count %d, free count:%d, archive count:%d",
                               ubi->works_count, ubi->free_count, ubi->tlc_archive_count);
			if (ubi->tlc_archive_count > 0) {
				struct ubi_work *wrk;
...
#ifdef CONFIG_MTK_SLC_BUFFER_SUPPORT
	if (e->tlc) {
		dbg_wl("[Bean]erase TLC PEB %d, old EC %llu, free count %d return directly\n",
...
try_update:
	...
	ubi->mtbl->info[idx].vol_id = cpu_to_be32(vol_id);
	ubi->mtbl->info[idx].map = cpu_to_be32(map);
	dbg_wl("change peb(%d)[%d] ec[%d] map[%d] count[%d]\n",
